(function(){
 ...
 const read=()=>{try{const saved=JSON.parse(localStorage.getItem(key)||'{}'),merged={...defaults,...saved},map=type=>['資訊技術專業證照','雲端／系統管理證照','資訊安全證照'].includes(type)?'資訊技術／資訊安全證照':type;Object.values(merged).forEach(rule=>{rule.required=[...new Set((rule.required||[]).map(map).filter(type=>types.includes(type)))];rule.recommended=[...new Set((rule.recommended||[]).map(map).filter(type=>types.includes(type)&&!rule.required.includes(type)))]});return merged}catch(e){return JSON.parse(JSON.stringify(defaults))}};
