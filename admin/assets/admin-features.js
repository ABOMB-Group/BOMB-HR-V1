...
(function(){
 ...
 function enhance(){
  ...
  if(currentRole()==='executive'&&active&&!active.protected&&/^(custom-|supervisor-custom-)/.test(ui.role)&&!document.querySelector('[data-delete-identity]')){const head=document.querySelector('.permission-role-head');if(head)head.insertAdjacentHTML('beforeend','<button class="secondary-btn identity-delete-btn" data-delete-identity="'+ui.role+'">刪除身份群組</button>')}
 ...
 document.addEventListener('click',event=>{const button=event.target.closest('[data-delete-identity]');if(!button)return;event.preventDefault();if(currentRole()!=='executive'){toast('只有最高權限可以刪除身份群組');return}const data=read(),id=button.dataset.deleteIdentity,role=data[id];if(!role||role.protected){toast('此身份為系統受保護身份，無法刪除');return}const people=window.BOMBHR_IMPORTED_PERSONNEL||[],assigned=(role.members||[]).map(employeeId=>people.find(person=>person.employeeId===employeeId)).filter(Boolean);if(assigned.length){openModal('無法刪除身份群組','此身份仍有已指派人員，請先到「人員指派／人員與部門」取消指派',`<div class="permission-locked"><b>尚有 ${assigned.length} 位人員</b><span>${assigned.map(person=>person.name+'・'+person.employeeId).join('、')}</span></div>`,`<button class="primary-btn" data-modal-close>我知道了</button>`);return}openModal('確認刪除身份群組','刪除後無法復原，既有稽核紀錄仍會保留',`<div class="danger-confirm"><b>${role.name}</b><span>目前未指派任何人員，可以安全刪除。</span></div>`,`<button class="secondary-btn" data-modal-close>取消</button><button class="primary-btn" data-confirm-delete-identity="${id}">確認刪除</button>`)},true);
 document.addEventListener('click',event=>{const button=event.target.closest('[data-confirm-delete-identity]');if(!button)return;event.preventDefault();if(currentRole()!=='executive'){toast('只有最高權限可以刪除身份群組');return}const data=read(),id=button.dataset.confirmDeleteIdentity,role=data[id];if(!role||role.protected)return;delete data[id];save(data);if(typeof addAudit==='function')addAudit('刪除身份群組',`${role.name}・${id}・未指派人員`);sessionStorage.setItem(uiKey,JSON.stringify({role:'owner',tab:'scope'}));closeModal();location.hash='permissions';window.dispatchEvent(new HashChangeEvent('hashchange'));toast('身份群組已刪除並留下稽核紀錄')},true);
