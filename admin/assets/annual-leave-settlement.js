(function(){
 ...
 function openAnnualHistory(employeeId){
  ...
  openModal('年假使用明細',`${employee.name}・${employeeId}｜依實際休假日期自動歸入月份`,`<div class="annual-history-summary"><div><small>本年度核發</small><b>${summary.rule.quota} 天</b></div><div><small>已使用</small><b>${summary.used} 天</b></div><div><small>目前剩餘</small><b>${summary.remaining} 天</b></div><div class="${pending?'has-unallocated':''}"><small>尚未歸入月份</small><b>${pending} 天</b></div></div>${pending?`<div class="annual-unallocated-warning"><div><b>有 ${pending} 天舊年假尚未歸入月份</b><small>額度已經扣除，補登日期不會再次扣除。</small></div>${allowed()?`<button class="primary-btn" data-annual-allocate="${employeeId}">補登使用日期</button>`:'<span>請由人事補登</span>'}</div>`:''}<div class="annual-history-note">已有日期但月份錯誤時，最高權限或財務人事行政可直接按「修正日期」；主管只能查看。</div><div class="annual-history-table">${history.length?`<table><thead><tr><th>使用月份</th><th>日期／期間</th><th>異動天數</th><th>來源</th><th>狀態</th><th>原因／操作人</th><th>操作</th></tr></thead><tbody>${history.map(item=>`<tr class="${item.status==='cancelled'||item.status==='reversed'?'is-reversed':''}"><td>${transactionMonth(item)}</td><td>${esc(item.period||item.usageDate||String(item.date||'').slice(0,10)||'—')}</td><td class="${Number(item.days)<0?'negative':''}">${Number(item.days)>0?'+':''}${Number(item.days||0)} 天</td><td>${esc(item.type||item.source||'系統紀錄')}</td><td><span>${transactionStatus(item)}</span></td><td><b>${esc(item.note||item.cancelReason||item.correctionReason||'—')}</b><small>${esc(item.operator||item.cancelledBy||item.correctedBy||'系統')}</small></td><td>${allowed()&&Number(item.days)>0&&!['cancelled','reversed','corrected'].includes(item.status)?`<button class="annual-correct-button" data-annual-correct="${esc(item.id)}" data-employee="${employeeId}">修正日期</button>`:'—'}</td></tr>`).join('')}</tbody></table>`:'<div class="annual-empty">目前尚無年假使用月份紀錄。請將舊資料補登日期，新的核准年假會自動出現在這裡。</div>'}</div>`,`<button class="secondary-btn" data-annual-history-back="${employeeId}">← 返回員工人事資料</button><button class="primary-btn" data-modal-close>完成</button>`);
 ...
 function openAnnualAllocation(employeeId,employeeName,pending){
  openModal('補登舊年假使用日期',`${employeeName}・目前有 ${pending} 天尚未歸入月份；本操作不會再次扣除額度`,`<div class="form-grid"><label class="form-field">補登天數<input id="annualArchiveDays" class="form-control" type="number" min=".5" step=".5" max="${pending}" value="${pending}"></label><label class="form-field">實際使用日期<input id="annualArchiveDate" class="form-control" type="date" value="${new Date().toISOString().slice(0,10)}"></label><label class="form-field full">補登原因<input id="annualArchiveReason" class="form-control" value="舊年假使用月份補登"></label></div><div class="policy-note">補登後會歸入選定月份並留下操作人與時間；已使用及剩餘天數不會重複變動。</div>`,`<button class="secondary-btn" data-annual-allocation-back>← 返回明細</button><button class="primary-btn" id="saveAnnualAllocation">確認補登</button>`);
  document.querySelector('[data-annual-allocation-back]').onclick=()=>openAnnualHistory(employeeId);
  document.getElementById('saveAnnualAllocation').onclick=()=>{const result=api.allocateAnnualHistory(employeeId,Number(document.getElementById('annualArchiveDays').value),document.getElementById('annualArchiveDate').value,document.getElementById('annualArchiveReason').value,`${currentProfile().name}・${currentProfile().id}`);if(!result.ok){toast(result.message);return}if(typeof addAudit==='function')addAudit('補登舊年假使用月份',`${employeeName}・${result.allocated} 天・${document.getElementById('annualArchiveDate').value}`);toast(`已補登 ${result.allocated} 天，年假額度沒有重複扣除`);openAnnualHistory(employeeId)};
 }
 function openAnnualCorrection(employeeId,employeeName,transactionId){
  ...
  const dates=String(item.period||item.usageDate||'').match(/\d{4}-\d{2}-\d{2}/g)||[],start=dates[0]||new Date().toISOString().slice(0,10),end=dates[1]||start;
  openModal('修正年假日期／月份',`${employeeName}・原紀錄 ${item.period||item.usageDate||'未記錄日期'}・${item.days} 天`,`<div class="form-grid"><label class="form-field">開始日期<input id="annualCorrectStart" class="form-control" type="date" value="${start}"></label><label class="form-field">結束日期<input id="annualCorrectEnd" class="form-control" type="date" value="${end}"></label><label class="form-field">使用天數<input id="annualCorrectDays" class="form-control" type="number" min=".5" step=".5" value="${item.days}"></label><label class="form-field full">修正原因<input id="annualCorrectReason" class="form-control" value="修正年假實際使用月份"></label></div><div class="policy-note">若只修正月份且天數不變，不會再次扣除額度；連動的請假來源與班表日期會一併更新。</div>`,`<button class="secondary-btn" data-annual-correct-back>← 返回明細</button><button class="primary-btn" id="saveAnnualCorrection">儲存修正</button>`);
  document.querySelector('[data-annual-correct-back]').onclick=()=>openAnnualHistory(employeeId);
  document.getElementById('saveAnnualCorrection').onclick=()=>{const startDate=document.getElementById('annualCorrectStart').value,endDate=document.getElementById('annualCorrectEnd').value,days=Number(document.getElementById('annualCorrectDays').value),reason=document.getElementById('annualCorrectReason').value.trim(),result=api.updateAnnualHistory(employeeId,transactionId,startDate,endDate,days,reason,`${currentProfile().name}・${currentProfile().id}`);if(!result.ok){toast(result.message);return}if(typeof addAudit==='function')addAudit('修正年假使用日期／月份',`${employeeName}・${result.before.period} → ${result.after.period}・${result.before.days} 天 → ${result.after.days} 天`);window.dispatchEvent(new CustomEvent('bombhr-demo-update'));toast(`已改至 ${startDate.slice(0,7).replace('-','／')}，年假、請假來源與班表已同步`);openAnnualHistory(employeeId)};
 }
