(function(){
 ...
 function transactionStatus(item){
  return item.status==='approved'?'已核准':item.status==='cancelled'?'已取消':item.status==='reversed'?'額度已恢復':item.status==='adjusted'?'人事已確認':item.status==='archived'?'舊紀錄已補登':item.status||'已記錄';
 }
