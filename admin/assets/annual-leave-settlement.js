(function(){
 ...
 function policyPanel(){
  ...
  return `<section class="annual-policy-shell">
   ...
   <div class="annual-policy-grid">
    ...
    <article><header><div><b>年度結算規則</b><small>法定年假未休完自動建立薪資加給</small></div></header><div class="annual-option-list"><label><span>結算方式<small>未休年假不直接作廢</small></span><select data-annual-mode><option value="cash" selected>折算工資</option></select></label><div class="annual-renewal-setting"><span>公司自動核發時間</span><div><label>月<input data-annual-reset-month type="number" min="1" max="12" value="${Number(p.resetMonth||1)}"></label><label>日<input data-annual-reset-day type="number" min="1" max="31" value="${Number(p.resetDay||1)}"></label><label>時間<input data-annual-reset-time type="time" value="${String(Number(p.resetHour||0)).padStart(2,'0')}:${String(Number(p.resetMinute||0)).padStart(2,'0')}"></label></div></div></div></article>
 ...
 function save(){
  ...
  const p=api.annualPolicy();document.querySelectorAll('[data-annual-group]').forEach(input=>p.groups[+input.dataset.annualGroup].quota=Number(input.value||0));p.employeeCanViewBalance=document.querySelector('[data-annual-balance]').checked;p.employeeCanViewSettlement=document.querySelector('[data-annual-settlement-visible]').checked;p.settlementMode='cash';p.resetMonth=Math.min(12,Math.max(1,Number(document.querySelector('[data-annual-reset-month]')?.value||1)));p.resetDay=Math.min(31,Math.max(1,Number(document.querySelector('[data-annual-reset-day]')?.value||1)));const resetTime=String(document.querySelector('[data-annual-reset-time]')?.value||'00:00').split(':');p.resetHour=Number(resetTime[0]||0);p.resetMinute=Number(resetTime[1]||0);api.saveAnnualPolicy(p);
 ...
 function enhanceEmployeeAnnualControls(employeeId){
  ...
  if(!grid.querySelector('.annual-grant-setting')){const profile=typeof employeeSalaryProfile==='function'?employeeSalaryProfile(employeeId):{},policy=api.annualPolicy(),mode=profile.annualGrantMode||'company',setting=document.createElement('div');setting.className='annual-grant-setting full';setting.innerHTML=`<label>年假自動核發<select id="employeeAnnualGrantMode" ${annualInput.disabled?'disabled':''}><option value="company" ${mode==='company'?'selected':''}>套用公司規則</option><option value="custom" ${mode==='custom'?'selected':''}>個人例外</option><option value="off" ${mode==='off'?'selected':''}>不自動核發</option></select></label><div class="annual-grant-custom"><label>核發月<input id="employeeAnnualGrantMonth" type="number" min="1" max="12" value="${Number(profile.annualGrantMonth||policy.resetMonth||1)}" ${annualInput.disabled?'disabled':''}></label><label>核發日<input id="employeeAnnualGrantDay" type="number" min="1" max="31" value="${Number(profile.annualGrantDay||policy.resetDay||1)}" ${annualInput.disabled?'disabled':''}></label><label>核發時間<input id="employeeAnnualGrantTime" type="time" value="${String(Number(profile.annualGrantHour??policy.resetHour??0)).padStart(2,'0')}:${String(Number(profile.annualGrantMinute??policy.resetMinute??0)).padStart(2,'0')}" ${annualInput.disabled?'disabled':''}></label></div>`;grid.insertBefore(setting,document.getElementById('employeeAnnualReason')?.closest('label')||null);const select=setting.querySelector('#employeeAnnualGrantMode'),sync=()=>setting.classList.toggle('use-company',select.value!=='custom');select.addEventListener('change',sync);sync()}
