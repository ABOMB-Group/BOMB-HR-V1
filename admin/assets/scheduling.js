...
(function(){
 const KEY='bombhr-schedules-v176',CODE_KEY='bombhr-schedule-codes-v182',CODE_ORDER_KEY='bombhr-schedule-code-order-v193',COMPANY_LAYOUT_KEY='bombhr-schedule-company-layout-v184';let pending=[],editMode=false,draft=null,selectedShift='09:00-18:00',selectedCode='1';
...
(function(){
 ...
 function policyLabel(setting={}){
  ...
  return `${modes[mode]||modes.roster}${mode==='roster'?'':`・${setting.shift||'09:00-18:00'}`}`;
 }
 function renderPolicyRows(filter=''){
  ...
  root.innerHTML=list.map(person=>{const setting=records[person.employeeId]||{},mode=setting.policyMode||(setting.mode==='fixed'?'weekend':'roster');return `<div class="schedule-policy-row" data-schedule-policy-row="${esc(person.employeeId)}"><div><b>${esc(person.name)}</b><small>${esc(person.employeeId)}・${esc(person.department||'—')}／${esc(person.position||'—')}</small><em>${esc(policyLabel(setting))}</em></div><label>排班制度<select class="form-control" data-policy-mode>${Object.entries(modes).map(([value,label])=>`<option value="${value}" ${mode===value?'selected':''}>${label}</option>`).join('')}</select></label><label data-policy-shift>預設班別<select class="form-control" data-policy-shift-value><option value="09:00-18:00" ${setting.shift==='09:00-18:00'||!setting.shift?'selected':''}>日班 09:00–18:00</option><option value="08:30-17:30" ${setting.shift==='08:30-17:30'?'selected':''}>早班 08:30–17:30</option><option value="13:00-22:00" ${setting.shift==='13:00-22:00'?'selected':''}>晚班 13:00–22:00</option><option value="19:00-07:00" ${setting.shift==='19:00-07:00'?'selected':''}>夜班 19:00–07:00</option></select></label></div>`}).join('')||'<div class="empty-state">找不到符合的員工。</div>';
  root.querySelectorAll('[data-policy-mode]').forEach(select=>{const refresh=()=>select.closest('[data-schedule-policy-row]').querySelector('[data-policy-shift]').hidden=select.value==='roster';select.onchange=refresh;refresh()});
 }
 function openPolicyManager(){
  ...
  openModal('員工排班制度','手動排班、臨時調班與調休永遠優先，不會被制度預設覆蓋',`<div class="schedule-policy-summary">${counts.map(([mode,count])=>`<span>${modes[mode]} <b>${count}</b></span>`).join('')}</div><label class="search-field">⌕<input id="schedulePolicySearch" placeholder="搜尋姓名、員編、部門或職位"></label><div class="schedule-policy-rows" id="schedulePolicyRows"></div><div class="schedule-participation-note"><b>套用原則：</b>排班制只有實際排入的日期算上班；固定週休會自動將六、日設為休；國定假日休依系統行事曆；任何人工修改都會保留並優先顯示。</div>`,`<button class="secondary-btn" data-modal-close>取消</button><button class="primary-btn" id="saveSchedulePolicies">儲存排班制度</button>`);
  renderPolicyRows();document.getElementById('schedulePolicySearch').oninput=event=>renderPolicyRows(event.target.value);
  document.getElementById('saveSchedulePolicies').onclick=()=>{const next=read();document.querySelectorAll('[data-schedule-policy-row]').forEach(row=>{const id=row.dataset.schedulePolicyRow,current=next[id]||{},policyMode=row.querySelector('[data-policy-mode]').value;next[id]={...current,mode:'participate',policyMode,shift:row.querySelector('[data-policy-shift-value]').value,updatedBy:`${currentProfile().name}・${currentProfile().id}`,updatedAt:new Date().toLocaleString('zh-TW',{hour12:false})}});write(next);if(typeof addAudit==='function')addAudit('修改員工排班制度',`${currentProfile().name}・${document.querySelectorAll('[data-schedule-policy-row]').length} 位員工`);toast('員工排班制度已儲存；既有及手動班表不受影響');openPolicyManager();window.dispatchEvent(new HashChangeEvent('hashchange'))};
 }
 function inject(){
